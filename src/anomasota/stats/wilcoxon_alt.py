...
def graph_ranks(avranks, names, p_values, cd=None, cdmethod=None, lowv=None, highv=None,
    ...
    # print(nnames)
    # for clq in cliques:
    for clq, c in zip(cliques, ['r', 'g', 'b', 'y', 'c',]):
        if len(clq) == 1:
            continue
        # print(clq)
        ...
        line([(rankpos(ssums[min_idx]) - side, start),
             ...
             linewidth=linewidth_sign, color=c,)
...
def form_cliques(p_values, nnames):
    ...
    cliques = [cliq for cliq in networkx.find_cliques(g) if len(cliq) > 1]
...
def wilcoxon_holm(alpha=0.05, df_perf=None):
    ...
    # average the ranks
    ranks_matrix = df_ranks.rank(ascending=False)
    average_ranks = ranks_matrix.mean(axis=1)
    
    print("average_ranks (before sorting)")
    print(average_ranks)
    
    average_ranks = average_ranks.sort_values(ascending=False)
    # return the p-values and the average ranks

    print("ranks_matrix.index.to_list()")
    print(ranks_matrix.index.to_list())

    print("ranks_matrix.columns.to_list()")
    print(ranks_matrix.columns.to_list())
    
    print('ranks_matrix.values.T')
    print(ranks_matrix.values.T)
